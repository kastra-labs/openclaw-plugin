...
const api = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === "/prefix/v1/evaluate") {
      if (active.apiError) { res.writeHead(active.apiError); res.end("{}"); return; }
      if (active.stallEvaluate) return;
      if (active.hold) {
        res.writeHead(202);
        res.end(JSON.stringify({ success: true, data: {
          decision: "HOLD", checkpoint_id: "cp-fixture", title: "Fixture", on_timeout: active.onTimeout ?? "DENY",
      ...
      res.end(JSON.stringify({ success: active.success ?? true, data: {
        decision, decision_id: "d-fixture", reason: "Fixture", matched_rule: { id: "r-fixture" },
      } })); return;
    }
    if (req.method === "GET") {
      ...
      res.end(JSON.stringify({ success: true, data: {
        id: active.wrongId ? "wrong" : "cp-fixture", status,
        effective_decision: status === "pending" ? undefined : active.effectiveDecision,
        decision_id: "d-hold", rule_id: "r-hold", resolved_by: active.resolvedBy,
...
try {
  for (const message of [false, true]) {
    for (const spec of cases) {
      ...
      const ctx = { channelId: "slack", accountId: "account", conversationId: "conversation", sessionKey: "agent:main:fixture", runId: "run-fixture" };
      if (message) {
      ...
      if (spec.disposition === "policy_allow") assert.equal(records[0].decisionId, "d-fixture");
